...
-- Last generated 2026-01-28 18:40:43
...
CREATE OR REPLACE VIEW v_mail_mails_list AS
    SELECT mail_mails_pkey, editnum, insby, insdatetime, modby, moddatetime, recipients, carboncopy, subject, message,mail_companies_fkey,mail_users_fkey,mail_status_fkey     FROM mail_mails;
...
CREATE OR REPLACE VIEW v_mail_status_list AS
    SELECT mail_status_pkey, editnum, insby, insdatetime, modby, moddatetime, status, description,mail_companies_fkey     FROM mail_status;

CREATE OR REPLACE VIEW v_mail_settings_list AS
    SELECT mail_settings_pkey, editnum, insby, insdatetime, modby, moddatetime, host, port, username, password,mail_companies_fkey     FROM mail_settings;
...
CREATE INDEX ind_companies_mails_companies_fkey
    ON companies_mails(companies_companies_fkey);
...
CREATE INDEX ind_users_mails_users_fkey
    ON users_mails(users_users_fkey);
...
CREATE INDEX ind_companies_status_companies_fkey
    ON companies_status(companies_companies_fkey);
...
CREATE INDEX ind_companies_settings_companies_fkey
    ON companies_settings(companies_companies_fkey);
